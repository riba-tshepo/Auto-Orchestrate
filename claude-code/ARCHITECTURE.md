...
**Components**: 17 agents | 48 skills | 1 command | 26 processes | 9 protocols | 2 templates | 9 CI engine modules | 4 domain memory modules
**Checkpoint schema**: `1.9.0` (parallel scheduling + 6-phase token-budget optimization)
...
- **Command** is `/auto-orchestrate` — the single user-facing autonomous loop controller. Audit (Phase 5v) and Debug (Phase 5e) run as internal sub-loops; domain analysis runs as Phases 5q/5s/5i/5d; release/post-launch/governance run as Phases 7/8/9.
...
┌──────────────────────────────────────────────────────────────────────────┐
│                           COMMAND LAYER                                 │
│  /auto-orchestrate  (single user-facing command)                         │
│    Internal phases: 1-4 planning · 0-6 execution · 5q/5s/5i/5d domain   │
│    · 5v audit · 5e debug · 7 release · 8 post-launch · 9 governance     │
│  /workflow-* (skills, not commands): start, end, dash, focus, next, plan │
├──────────────────────────────────────────────────────────────────────────┤
│                            AGENTS LAYER                                 │
│  orchestrator │ researcher     │ session-manager│ debugger                │
│  auditor      │ data-engineer  │ engineering-manager│ infra-engineer       │
...
├──────────────────────────────────────────────────────────────────────────┤
│                            SKILLS LAYER                                 │
│  48 specialized skills (each with SKILL.md + optional Python scripts)   │
├────────────────────────────────┬─────────────────────────────────────────┤
...
### Token-Budget Optimization Suite (6 phases, ~45% session reduction)

A typical `/auto-orchestrate` session consumed ~2.0M tokens before optimization; the 6-phase suite reduces this to ~1.1M (~45% saving). Every phase is gated behind a `checkpoint.optimizations.<flag>` boolean — default `true` on fresh installs, `false` on resumed sessions to keep prior on-disk receipts compatible. All 6 are reversible without code change.

```
┌────────────────────────────────────────────────────────────────────────┐
│  Phase 1 — SKILL-FRONTMATTER-001  (~76k saved)                        │
│   Discovery loads SKILL.md frontmatter only (~300 tok); body at invoke │
│   Helper: layer1.read_frontmatter / list_skills_with_triggers          │
├────────────────────────────────────────────────────────────────────────┤
│  Phase 2 — process_injection_slim  (~50k saved)                       │
│   Spawn-prompt builder injects only fired hooks, not the full map      │
│   [INJECT-AUDIT] log surfaces under-injection                          │
├────────────────────────────────────────────────────────────────────────┤
│  Phase 3 — MANIFEST-DIGEST-001  (~416k saved)                         │
│   Subagents get 2.6k digest (agents+triggers) vs 19k full manifest    │
│   orchestrator + session-manager + needs_full_manifest=true → full     │
│   Helper: layer1.build_digest / needs_full_manifest                    │
├────────────────────────────────────────────────────────────────────────┤
│  Phase 4 — TEMPLATE-EXTRACT-001  (~302k saved)                        │
│   Orchestrator spawns get CORE (~8k) + active stage/phase template     │
│   instead of full 33k orchestrator.md; safe fallback to full file      │
│   Helper: layer1.build_spawn_prompt_body                               │
├────────────────────────────────────────────────────────────────────────┤
│  Phase 5 — STAGE-RECEIPT-DIET-001  (~40k saved)                       │
│   Slim v2.0.0 stage-receipt drops command, duration_seconds,           │
│   manifest_entries, domain_memory_writes, retry_count; tolerant reads  │
├────────────────────────────────────────────────────────────────────────┤
│  Phase 6 — HANDOVER-COMPRESS-001  (~20k saved)                        │
│   Slim v2.0.0 handover-receipt drops context_carry, confidence,        │
│   consumed/consumed_at; consumer re-derives from checkpoint            │
└────────────────────────────────────────────────────────────────────────┘
```

**Instrumentation (Phase 0)**: Per-spawn token counters live in `iteration_history[].token_counts_by_spawn` and aggregate to `checkpoint.session_token_total`. Estimation method documented in `commands/auto-orchestrate.md` Step 4.6 (chars / 4).

**Schema bumps**: `1.6.0 → 1.7.0` adds parallel scheduling fields; `1.7.0 → 1.8.0` adds first 4 optimization flags + token counters; `1.8.0 → 1.9.0` adds `stage_receipt_diet` + `handover_compress` flags. Each migration block defaults the new flags to `false` for resumed sessions; new sessions get `true`.

### Parallel Stage 3 Implementation (PARALLEL-001/002/003 + CHAIN-002)

When the product-manager (Stage 1) emits `independence_groups` and `dependency_graph` in `proposed-tasks.json`, the orchestrator's Stage 3 may spawn up to **5 concurrent software-engineers** (configurable 1–7 via `checkpoint.parallel_cap`) — one task per distinct independence group per spawn cycle.

**Hybrid independence detection** (per `agents/product-manager.md`):
- **Heuristic (default)**: tasks sharing a common path prefix (depth ≤ 2) of `files_touched` default to the same group.
- **Explicit override (always wins)**: spec/task fields `independence_groups: [[ids],...]`, `shares_state_with: [ids]`, `independent_of: [ids]`.

**Per-group stage tracking**: `checkpoint.independence_group_stages: {group_id: stage_n}` lets independent groups advance through stages at different rates without violating CHAIN-001 (which is relaxed cross-group per PARALLEL-002).
...
**Internal Phase Activation (Phase 5e — Debug sub-loop)**:
- **From Stage 5**: When the validator exhausts 3 fix iterations and errors persist, the loop controller transitions internally to Phase 5e per PHASE-LOOP-001. No human pause; no escalation. The debugger agent runs the triage-research-fix-verify cycle; on success, control returns to Stage 5 for re-validation. Cap: 2 Stage 5 → Phase 5e cycles per session (matches REGRESS-002).
- **From Phase 5v (Audit)**: When the gap report contains `implementation_error` or `runtime_failure` gaps, the loop controller transitions to Phase 5e internally before re-attempting the audit cycle.
...
The 12 team agents each handle a specialized engineering domain. They are spawned by `/auto-orchestrate` at the appropriate internal phase per AUTO-001 (phase-determined agent gateway). Each agent has:
...
| `infra-engineer` | Developer platform + cloud infrastructure (consolidated platform/cloud) | Infrastructure & Platform (P-044 to P-048) |
...
All 12 agents are registered in `manifest.json` under `agents[]` with `dispatch_triggers` and `skills_orchestrated`.
...
| 7. Infrastructure & Platform | P-044 to P-048 | infra-engineer |
...
Process ownership is defined in `processes/AGENT_PROCESS_MAP.md`. Each of the 12 team agents has Dispatch Triggers and Process Ownership sections defined in their `.md` files. Agents own 2-22 processes and support 0-38 additional processes.
...
**Core Constraints (AUTO-001 to AUTO-007, CEILING-001, CHAIN-001/002, PARALLEL-001/002/003, PROGRESS-001, DISPLAY-001, SCOPE-001, SCOPE-002, plus 6 token-budget optimization constraints)**:
...
| CHAIN-002 | Independence exception — independent task groups may progress through stages concurrently (PARALLEL-002 relaxes the chain) |
| PARALLEL-001 | Stage 1 product-manager emits `independence_groups` + `dependency_graph` via hybrid heuristic + explicit-override detection |
| PARALLEL-002 | Cross-group stage relaxation — tasks in different independence groups may run at different stages concurrently |
| PARALLEL-003 | Concurrency cap — max 5 concurrent Stage 3 spawns (configurable up to 7 via `checkpoint.parallel_cap`) |
...
| SKILL-FRONTMATTER-001 | Skill discovery loads YAML frontmatter only (~300 tok); body loads only at invocation |
| MANIFEST-DIGEST-001 | Subagents receive 2.6k digest instead of 19k full manifest; orchestrator + session-manager + `needs_full_manifest:true` get full |
| TEMPLATE-EXTRACT-001 | Orchestrator spawns receive CORE + active stage/phase/meeting template instead of full 33k orchestrator.md |
| STAGE-RECEIPT-DIET-001 | Slim v2.0.0 stage-receipt schema; consumers tolerant of both v1 and v2 |
| HANDOVER-COMPRESS-001 | Slim v2.0.0 handover-receipt schema; `context_carry` re-derived from checkpoint by tolerant readers |
...
All 48 skills reference `skill-boilerplate.md` sections:
...
### 10.1 Single-Command Internal Phase Architecture

`/auto-orchestrate` is the only user-facing command. The previously separate autonomous loops (`/auto-debug`, `/auto-audit`) and lifecycle/domain commands (`/new-project`, `/qa`, `/security`, etc.) are now internal phases of this one command.

```
┌─────────────────────────────────────────────────────────────────────────┐
│                       /auto-orchestrate <task>                          │
│                                                                         │
│  Phase 1 → Phase 2 → Phase 3 → Phase 4 ──────► Stages 0-6 (Execution) │
│  Intent     Scope     Dep'cy   Sprint           ▲      │                │
│                                                  │      ▼                │
│                                            ┌─────┴─────────────┐         │
│                                            │  Domain phases    │         │
│                                            │  5q/5s/5i/5d      │         │
...
│                                          Phase 9 (Governance)            │
│                                                                         │
│  All phases write to: .orchestrate/<session>/                           │
│  Cross-session knowledge: .domain/                                      │
│                                                                         │
│  WORKFLOW SKILLS (not commands): /workflow-plan, /workflow-start,        │
│    /workflow-end, /workflow-dash, /workflow-focus, /workflow-next       │
└─────────────────────────────────────────────────────────────────────────┘
```
...
|----------|-------|-------|
| Command | `/auto-orchestrate` | Single user-facing entry point |
| Skills | `/workflow-*` (6 skills) | Read-only access to session state via `task-board.json` |
| Internal phases | Phase 1-9 (incl. 5q/5s/5i/5d/5v/5e) | Internal phase invocation per PHASE-LOOP-001 |
| Shared knowledge | `.domain/`, `.pipeline-state/` | Cross-session knowledge, persistent across multiple auto-orchestrate runs |
...
|------|---------|-----------------|
| `skill-boilerplate.md` | 7-step execution sequence, manifest entry format | 68+ references across 48 skills |
...
- [ ] All 17 agents documented (2 pipeline-core: orchestrator, researcher + 3 pipeline: debugger, auditor, session-manager + 12 team agents)
- [ ] All 48 skills cataloged
- [ ] Single user-facing command (`/auto-orchestrate`) referenced; legacy `/auto-debug`, `/auto-audit`, `/release-prep`, `/post-launch`, etc. are now internal phases (5e, 5v, 7, 8) of `/auto-orchestrate`
...
└── skills/
    └── _shared/
        └── python/
            ...
            │   ├── manifest_digest.py    # Slim manifest builder (MANIFEST-DIGEST-001)
            │   ├── memory.py             # Memory management utilities
            │   ├── orchestrator_template.py  # Section extractor (TEMPLATE-EXTRACT-001)
            │   ├── output_format.py      # JSON/human output formatting
            │   ├── skill_frontmatter.py  # YAML frontmatter reader (SKILL-FRONTMATTER-001)
            │   └── spec_utils.py         # Spec keyword/priority/type inference
            ├── layer2/
            │   ├── __init__.py
...
│   logging.py │ memory.py │ output_format.py │ spec_utils.py    │
│   skill_frontmatter.py (SKILL-FRONTMATTER-001)                  │
│   manifest_digest.py    (MANIFEST-DIGEST-001)                   │
│   orchestrator_template.py (TEMPLATE-EXTRACT-001)               │
│                                                                 │
...
> **SUPERSEDED 2026-04-25**: The bridge protocol described in this section connected the formerly separate `/new-project` command to `/auto-orchestrate`. As of the 2026-04-25 consolidation, those are the same command — the planning pipeline runs as Phases 1-4 inside `/auto-orchestrate` with auto-evaluated gates (no human pause). The gate state system is preserved as an audit-trail artifact (`gate-state.json`) but is no longer written by `/gate-review` (deleted) — the auto-orchestrate loop controller writes it directly per AUTO-EVAL-001.
>
> The remainder of this section is retained as historical context for sessions that pre-date the consolidation. For the current single-command architecture, see Section 4 (Agents) and Section 10 (Component Relationships) above.
